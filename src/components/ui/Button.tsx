...
const variants = {
  primary: 'brand-primary-button bg-[#01D439] text-white shadow-glow hover:bg-[#01D439]',
...
export default function Button({ href, children, variant = 'primary', icon = false }: ButtonProps) {
  ...
  const content = (
    <>
      {variant !== 'primary' ? <span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition duration-400 group-hover:translate-x-full" aria-hidden="true" /> : null}
