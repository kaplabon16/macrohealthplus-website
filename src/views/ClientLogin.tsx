    ...
    >
      ...
      >
        {products.map((product) => (
          <motion.a
            className="group flex min-h-20 items-center justify-between gap-3 border-b border-white/15 px-4 py-4 transition duration-200 hover:bg-[#01D439]/[0.06] sm:min-h-24 sm:border-r sm:px-5 lg:min-h-28 lg:py-6"
            href={product.href}
            key={product.title}
            variants={{ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } } }}
          >
            <h2 className="text-base font-semibold text-white transition duration-200 group-hover:text-[#01D439] sm:text-lg">{product.title}</h2>
            <ArrowUpRight className="h-5 w-5 text-slate-400 transition duration-200 group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-[#01D439]" aria-hidden="true" />
          </motion.a>
        ))}
      </motion.div>
    </Section>
